<?php if (isset($post['comment_post']) && is_array($post['comment_post'])): ?>
    ...
    <?php if (!empty($post['comment_post']['type']) && $post['comment_post']['type'] == 'disqus'): ?>
        <?php echo $post['comment_post']['data'] ?? ''; ?>
    <?php endif; ?>

    <?php if (!empty($post['comment_post']['type']) && $post['comment_post']['type'] == 'system'): ?>
        <div id="post-comment" class="post-comment">
            ...
            <h4>
                <?php if (empty($post['comment_post']['data'])): ?>
                    Leave a comment
                <?php else: ?>   
                    <?php echo $post['comment_post']['count'] ?? 0 ?> Comments
                <?php endif ?>
            </h4>

            <div class="comment-item mt-4 mb-5">
                <?php  
                if (!empty($post['comment_post']['data']) && is_array($post['comment_post']['data'])) {
                            ...
                            foreach ($comment as $post_comment) {
                                ...
                                <div class="wall-comment mb-0 mt-10">
                                    <span class="avatar float-left">
                                        <img alt="<?php echo $post_comment['name'] ?? 'Guest' ?>" 
                                             src="<?php echo base_url('storage/blog/pisen/img/person.png') ?>" 
                                             title="<?php echo $post_comment['name'] ?? 'Guest' ?>">
                                    </span>
                                    <div class="ml-50 comment-block" 
                                         id="comment-<?php echo $post_comment['id'] ?? '' ?>" 
                                         <?php echo (isset($post_comment['status']) && $post_comment['status'] == 'Blocked') ? 'style="background: #ffdce1;"' : '' ?>>
                                        <div class="media-body text-middle">
                                            <h4 class="comment-name">
                                                <?php echo $post_comment['name'] ?? 'Anonymous' ?>
                                            </h4>
                                    ...
                                    <div class="dropdown comment-operation">
                                        <a class="comment-reply-link badge badge-light" 
                                           comment-id="comment-<?php echo $post_comment['id'] ?? '' ?>" 
                                           data-comment-id="<?php echo $post_comment['id'] ?? '' ?>">
                                           <i class="fa fa-reply"></i>
                                        </a>
                                    </div>
                                </div>  
                                <?php
                                if (!empty($post_comment['child']) && is_array($post_comment['child'])) {
                                    foreach ($post_comment['child'] as $post_comment_child) {
                                        ?>
                                        <div class="wall-comment ml-50 mt-10">
                                            <span class="avatar float-left">
                                                <img alt="<?php echo $post_comment_child['name'] ?? 'Guest' ?>" 
                                                     src="<?php echo base_url('storage/blog/pisen/img/person.png') ?>" 
                                                     title="<?php echo $post_comment_child['name'] ?? 'Guest' ?>">
                                            </span>
                                            <div class="ml-50 comment-block" 
                                                 id="comment-<?php echo $post_comment_child['id'] ?? '' ?>" 
                                                 <?php echo (isset($post_comment_child['status']) && $post_comment_child['status'] == 'Blocked') ? 'style="background: #ffdce1;"' : '' ?>>
                                                <div class="media-body text-middle">
                                                    <h4 class="comment-name">
                                                        <?php echo $post_comment_child['name'] ?? 'Anonymous' ?>
                                                    </h4>
                                                    <small class="text-muted">
            ...
            <form id="form-comment" method="POST">
                ...
                if ($this->session->flashdata('comment')) {
                    if ($this->session->flashdata('comment') == 'success_pending') {
                        ...
                        <div class="alert alert-warning alert-dismissable">
                            <i class="fa fa-info-circle"></i> Success send comment, will show after approved by administrator
                        </div>
                        <?php
                        ...
                        <div class="alert alert-danger alert-dismissable">
                            <i class="fa fa-info-circle"></i> Failed send comment, try again later
                        </div>
                        <?php
                    }
                }
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-6">
                        <input value="" class="input-form trans-bg" name="email" type="email" placeholder="Email" required="">
                    </div>
                </div>
